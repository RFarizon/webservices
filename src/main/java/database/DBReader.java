...
public class DBReader {
  ...
  public List<Property> selectProperty(String address, int zip) {
    try {
      ...
      while (rs.next()) {
        System.out.println("Property found. Adding property to list.");
        ...
        if (rs.getString(17) == null) {
          ...
          p.setYearBuilt(year);
          // p.setYearBuilt(rs.getString(11).substring(0, 3));
        }
  ...
  public Zestimate selectZestimate(BigInteger zpid) {
    try {
      ...
      PreparedStatement ps = this.connect.prepareStatement(st, ResultSet.TYPE_SCROLL_INSENSITIVE,
  ...
  public List<TaxAssessment> selectTaxAssessments(BigInteger zpid) {
    try {
      ...
      ResultSet rs = null;
      String st = "SELECT * FROM TaxAssessments T WHERE T.zpid = ?" + " ORDER BY taxYear DESC;";
  ...
  public List<ZillowComparable> selectZillowComparables(BigInteger zpid) {
    try {
      ...
      ResultSet rs = null;
      String st =
          "SELECT * FROM Comparables C WHERE C.primaryZPID = ?" + " ORDER BY compScore ASC;";
